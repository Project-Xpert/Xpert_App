...
const Signup = () => {
  ...
  const isValidData = () => {
    ...
    if (userId.length < 3 || userId.length > 20) {
      ...
      newErrorMessages.userId = "유저 아이디에는 영문자 대소문자, 숫자, 언더바(_)만 허용됩니다"
      isValid = false
    } else {
    ...
    if (password.length < 8 || password.length > 20) {
      newErrorMessages.password = "비밀번호는 8글자 이상 20글자 이하여야 합니다."
      isValid = false
    } else if (!password.match(passwordValidation)) {
      newErrorMessages.password = "비밀번호에는 숫자, 영문자, 특수문자가 하나씩은 포함되어야 합니다"
      isValid = false
  ...
  const signup = () => {
    if (isValidData()) {
      ...
      .catch(e => {
        ...
        } else if(e.response?.data.description == "이미 해당 이메일로 가입된 유저가 존재합니다") {
          setErrorMessage({
            userId: "",
